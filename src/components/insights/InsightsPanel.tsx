...
export function InsightsPanel({ tokenId, tokenName, tokenColor = '#627EEA', isOpen, onClose }: InsightsPanelProps) {
    ...
    if (error || !insights) {
      return (
        <div className="p-6 text-center">
          <p className="text-gray-400">{error || 'No insights available'}</p>
        </div>
      );
    }
      ...
      <>
        ...
        <div className="bg-[#2b3139] rounded-xl p-5 mt-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-gray-500 text-sm">{formatTimeAgo(insights.updated_at)}</span>
          </div>
          {insights.tldr && (
            <>
              <p className="text-[#eaecef] text-[15px] leading-7 mb-5">{insights.tldr.summary}</p>
              <ol className="space-y-4">
                {insights.tldr.key_points.map((point, i) => (
                  <li key={i} className="flex gap-3 text-sm text-[#b7bdc6] leading-relaxed">
                    <span className="text-gray-500 font-medium flex-shrink-0">{i + 1}.</span>
                    <span>{point}</span>
                  </li>
                ))}
              </ol>
        ...
        {insights.positives.map((item, i) => (
          <div key={i} className="mb-5">
            ...
            <span className="text-green-500 font-semibold text-sm">{item.category}: </span>
            <span className="text-sm text-[#b7bdc6] leading-relaxed">{item.description}</span>
          </div>
        ))}
        ...
        {insights.negatives.map((item, i) => (
          <div key={i} className="mb-5">
            <div className="text-gray-500 text-sm mb-1">{i + 1}.</div>
            <span className="text-red-500 font-semibold text-sm">{item.category}: </span>
            <span className="text-sm text-[#b7bdc6] leading-relaxed">{item.description}</span>
          </div>
        ))}
        ...
        <div className="my-6">
          ...
          >
            ...
            <span className="bg-yellow-500 text-[#0b0e11] text-xs font-semibold px-2.5 py-1 rounded-full">
              {insights.sources.length} articles
            </span>
          </button>
        </div>
        ...
        <div className="flex items-center gap-2 text-base font-semibold mt-7 mb-4 text-[#eaecef]">
          ...
          Discussion Topics
          <span className={`text-xs px-2.5 py-1 rounded ml-1 capitalize ${
            insights.discussion.overall === 'positive' ? 'bg-green-500/15 text-green-500' :
            insights.discussion.overall === 'negative' ? 'bg-red-500/15 text-red-500' :
            'bg-gray-500/15 text-gray-500'
          }`}>
            {insights.discussion.overall}
          </span>
        </div>
        <ul className="space-y-3">
          {insights.discussion.topics.slice(0, 3).map((topic, i) => (
            <li key={i} className="flex items-start gap-2 text-sm text-[#b7bdc6] leading-relaxed">
        ...
        <div className="mt-4">
          ...
          >
            ...
            <span className="bg-yellow-500 text-[#0b0e11] text-xs font-semibold px-2.5 py-1 rounded-full">
              {insights.discussion.topics.length} topics
            </span>
          </button>
        </div>
            ...
            <div className="bg-[#2b3139] rounded-xl p-5 mb-4">
              <div className="flex items-center justify-between mb-4">
                ...
                <span className="text-xl font-bold text-[#eaecef]">
                  ${insights.technical_analysis.current_price.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                </span>
              </div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  ...
                  <div className="text-center">
                    ...
                    <div className={`text-lg font-semibold ${
                      insights.technical_analysis.indicators.rsi > 70 ? 'text-red-400' :
                      insights.technical_analysis.indicators.rsi < 30 ? 'text-green-400' :
                      'text-[#eaecef]'
                    }`}>
                      {insights.technical_analysis.indicators.rsi.toFixed(1)}
                    </div>
                    <div className="text-[10px] text-gray-600">
                      {insights.technical_analysis.indicators.rsi > 70 ? 'Overbought' :
                       insights.technical_analysis.indicators.rsi < 30 ? 'Oversold' : 'Neutral'}
                    </div>
                  </div>
                  ...
                  <div className="text-center">
                    ...
                    <div className={`text-lg font-semibold ${
                      insights.technical_analysis.indicators.macd.signal === 'bullish' ? 'text-green-400' :
                      insights.technical_analysis.indicators.macd.signal === 'bearish' ? 'text-red-400' :
                      'text-[#eaecef]'
                    }`}>
                      {insights.technical_analysis.indicators.macd.signal === 'bullish' ? '↑' :
                       insights.technical_analysis.indicators.macd.signal === 'bearish' ? '↓' : '—'}
                    </div>
                  ...
                  <div className="text-center">
                    ...
                    <div className={`text-lg font-semibold ${
                      insights.technical_analysis.indicators.volume.ratio > 1.2 ? 'text-green-400' :
                      insights.technical_analysis.indicators.volume.ratio < 0.8 ? 'text-red-400' :
                      'text-[#eaecef]'
                    }`}>
                      {insights.technical_analysis.indicators.volume.ratio.toFixed(2)}x
                    </div>
                    <div className="text-[10px] text-gray-600">
                      vs 7d avg
                    </div>
              ...
              {insights.technical_analysis.indicators && (
                <div className="mt-4 pt-4 border-t border-[#3c4451]">
                  <div className="grid grid-cols-2 gap-4 text-xs">
                    <div>
                      <div className="text-gray-500 mb-2">EMA Levels</div>
                      <div className="space-y-1">
                        <div className="flex justify-between">
                          <span className="text-gray-600">EMA 7</span>
                          <span className="text-[#b7bdc6]">${insights.technical_analysis.indicators.ema.short_7.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">EMA 25</span>
                          <span className="text-[#b7bdc6]">${insights.technical_analysis.indicators.ema.medium_25.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">EMA 99</span>
                          <span className="text-[#b7bdc6]">${insights.technical_analysis.indicators.ema.long_99.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                        </div>
                      </div>
                    </div>
                    <div>
                      <div className="text-gray-500 mb-2">Bollinger Bands</div>
                      <div className="space-y-1">
                        <div className="flex justify-between">
                          <span className="text-gray-600">Upper</span>
                          <span className="text-[#b7bdc6]">${insights.technical_analysis.indicators.bb.upper.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Lower</span>
                          <span className="text-[#b7bdc6]">${insights.technical_analysis.indicators.bb.lower.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Width</span>
                          <span className="text-[#b7bdc6]">{insights.technical_analysis.indicators.bb.width_percent.toFixed(1)}%</span>
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            {insights.technical_analysis.positive_paragraphs.length > 0 && (
              <div className="bg-[#2b3139] rounded-xl p-5 mb-4">
                ...
                <ul className="space-y-2">
                  {insights.technical_analysis.positive_paragraphs.map((para, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm text-[#b7bdc6] leading-relaxed">
                      <span className="w-1.5 h-1.5 bg-green-500 rounded-full mt-2 flex-shrink-0"></span>
                      {para}
                    </li>
        ...
        <div className="bg-[#2b3139] rounded-xl p-5 mt-6">
          <div className="flex items-center justify-between mb-3">
            <span className="text-sm font-semibold text-[#eaecef]">Sentiment Score</span>
            <span className={`text-2xl font-bold ${
              insights.discussion.score > 0 ? 'text-green-500' :
              insights.discussion.score < 0 ? 'text-red-500' :
              'text-gray-400'
            }`}>
              {insights.discussion.score > 0 ? '+' : ''}{insights.discussion.score.toFixed(2)}
            </span>
          </div>
          {/* Scale bar: -5 to +5 */}
          <div className="relative w-full h-2 bg-[#3c4451] rounded-full mb-2">
            ...
            <div
              ...
              style={{
                left: insights.discussion.score >= 0 ? '50%' : `${50 + (insights.discussion.score / 5) * 50}%`,
          ...
          <div className="text-xs text-[#5e6673]">
            Based on {insights.discussion.tweet_count} tweets analyzed
          </div>
        </div>
      </>
    );
      ...
      <>
        ...
        <div className="mb-5">
          <div className="flex items-center gap-2">
            <span className="text-lg font-semibold text-[#eaecef]">Discussion Topics</span>
            <span className={`text-xs px-2.5 py-1 rounded capitalize ${
              insights.discussion.overall === 'positive' ? 'bg-green-500/15 text-green-500' :
              insights.discussion.overall === 'negative' ? 'bg-red-500/15 text-red-500' :
              'bg-gray-500/15 text-gray-500'
            }`}>
          ...
          <div className="text-sm text-gray-500 mt-1">
            {insights.discussion.topics.length} topics • {insights.discussion.tweet_count} tweets
          </div>
        </div>
        {insights.discussion.topics.map((topic, i) => (
          <div key={i} className="bg-[#2b3139] rounded-lg p-4 mb-3">
            <div className="text-sm font-semibold text-[#eaecef] leading-snug">{topic}</div>
          </div>
        ))}
            ...
            {insights.discussion.top_tweets.map((tweet, i) => (
              <div key={i} className="bg-[#2b3139] rounded-lg p-4 mb-3">
                <div className="flex items-center gap-2 mb-2">
                  <span className="text-sm font-semibold text-[#eaecef]">{tweet.author_name}</span>
                  ...
                  <span className="text-xs text-gray-500">@{tweet.author_username}</span>
                </div>
                <div className="text-sm text-[#b7bdc6] leading-relaxed mb-3">{tweet.text}</div>
  ...
  return (
    <>
      ...
      {/* Side Panel */}
      <div className={`fixed top-0 right-0 w-[480px] max-w-full h-screen bg-[#1e2329] z-[1000] flex flex-col transform transition-transform ${isOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        {/* Header */}
        <div className="flex items-center px-6 py-5 border-b border-[#2b3139] flex-shrink-0">
          <div
            className="w-10 h-10 rounded-full flex items-center justify-center mr-3"
            style={{ background: `linear-gradient(135deg, ${tokenColor}, #3c3c3d)` }}
          >
          ...
          <h1 className="text-xl font-semibold text-[#eaecef] flex-1">{tokenName} Insights</h1>
          <button
            onClick={onClose}
